...
class EPIDataSetTrain(data.Dataset):
    def __init__(self, root, name, cv):
        ...
        enhancer, promoter, label = train['enhancer'], train['promoter'], train['label']
        self.enhancer = enhancer.transpose((0, 2, 1))
        self.promoter = promoter.transpose((0, 2, 1))
        self.label = label
...
class EPIDataSetTest(data.Dataset):
    def __init__(self, root, name, cv):
        ...
        enhancer, promoter, label = test['enhancer'], test['promoter'], test['label']
        self.enhancer = enhancer.transpose((0, 2, 1))
        self.promoter = promoter.transpose((0, 2, 1))
        self.label = label
